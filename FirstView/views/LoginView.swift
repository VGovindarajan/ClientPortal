...
public struct LoginView: View {
    ...
    @State private var showMainView = false // Control presentation
    @State private var navigateToMainView = false // Control presentation
    ...
    public var body: some View {
        VStack {
            ...
            Image("name")
                ...
                .frame(width: 384, height: 384) // Set a specific size (adjust as needed)
                .padding(.all, 16) // Add some spacing to the right
            Text("FCM Client Portal")
                .padding([.all],16)
                .font(.system(size: 18, weight: .bold, design: .default)) // Font styling
            ...
            Button(action: {
                ...
                Task {
                    await
                    apiClient.login(username: userName, password: password) { (result: Result<User,ApiError>) in
                        DispatchQueue.main.async {
                            ...
                            case .failure(let error):
                                let _ = apiClient.setErrorMessage(message: error.localizedDescription) // Set the error message
        ...
        .padding()
        .navigationDestination(isPresented: $navigateToMainView) { // For Navigation
            if let _user = user {
                MainView(user: _user)
